...
class ConfigManager:
    ...
    @staticmethod
    def load_config():
        config_template = Path(__file__).resolve().parent.parent / 'templates' / 'config.example.json'
        ...
        config = FileManager.load_json_file(f"{Path.cwd()}/config.json", object_pairs_hook=collections.OrderedDict)
    ...
    @staticmethod
    def manual_config():
        logging.info("Hello and welcome, it looks like you don't have a config file (yet)")
        config_template = Path(__file__).resolve().parent.parent / 'templates' / 'config.example.json'
        if not FileManager.path_exists(config_template):
            logging.error("Oh no, config.example.json and config.json do not exist. You broke something didn't you?")
            return False
        ...
        if input("Do you understand this and still wish to continue, please type: yes and press enter> ").lower() != "yes":
            ...
            sys.exit(0)
        config_template = Path(__file__).resolve().parent.parent / 'templates' / 'config.example.json'
        template = FileManager.load_json_file(config_template, object_pairs_hook=collections.OrderedDict)
        if not template:
